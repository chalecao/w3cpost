...
正向代理很好理解，比如手机上wifi设置的代理就是正向代理，[浏览器](https://www.w3cdoc.com)设置的代理也是正向代理，正向代理只是简单的为你转发请求，把请求到的数据回传到源请求。
...
反向代理可以看成一个服务器，比如[我们](https://www.w3cdoc.com)常用的nginx反向代理，服务器上只是监听了一个80端口，但是这个80端口既提供网络服务，又提供接口服务，可以通过不同的域名或者请求路径区分开不同的服务，这就用到了反向代理。对于客户端只是向反向代理请求数据，至于反向代理是自身的服务，还是其他组合的服务，客户端并不知情。

## node实现正向代理

在正向代理中，[我们](https://www.w3cdoc.com)通常会使用两种形式的代理：
...
好的工具需要有正确的使用姿势。
  ...
  1. 如果你需要调试的接口都已上线，你只是简单的想代理到本地，那么可以将静态文件代理到本地即可。这时候用到的是正向代理，可能需要[浏览器](https://www.w3cdoc.com)插件配置http代理到本地的代理服务器，然后通过配置文件配置需要代理的静态资源。
